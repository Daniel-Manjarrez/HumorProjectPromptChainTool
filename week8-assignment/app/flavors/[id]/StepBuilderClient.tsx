...
export default function StepBuilderClient({ flavorId, initialSteps, lookups }: Props) {
  ...
  const getStepTypeName = (id: number) => lookups.stepTypes.find(t => t.id === id)?.name || 'Unknown';
                        ...
                        <div className="flex-grow flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
                          <div>
                            <div className="flex items-center gap-3 mb-1">
                              <span className="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs font-bold px-2 py-1 rounded-md">
                                Step {index + 1}
                              </span>
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400 truncate max-w-xl">
                              {step.description || step.llm_system_prompt || 'No description'}
                            </p>
                          </div>

                          <div className="flex gap-2 shrink-0">
                            <button onClick={() => handleOpenEdit(step)} className="p-2 text-gray-500 hover:text-blue-600 dark:hover:text-blue-400 bg-gray-50 dark:bg-gray-700/50 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-md transition-colors">
                              <Pencil className="h-4 w-4" />
                            </button>
                            <button onClick={() => handleDelete(step.id)} className="p-2 text-gray-500 hover:text-red-600 dark:hover:text-red-400 bg-gray-50 dark:bg-gray-700/50 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-md transition-colors">
            ...
            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  >
                    {lookups.stepTypes.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                  </select>
                </div>
                ...
                <div>
                  ...
                  >
                    {lookups.inputTypes.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                  </select>
                </div>
                ...
                <div>
                  ...
                  >
                    {lookups.outputTypes.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={4}
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 bg-white dark:bg-gray-900 text-gray-900 dark:text-white font-mono text-sm"
